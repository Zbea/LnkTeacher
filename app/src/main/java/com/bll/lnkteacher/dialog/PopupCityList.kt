    ...
    fun builder(): PopupCityList{
        ...
        for (i in provinces.indices){
            val province=provinces[i]
                if (province.value==MethodManager.getUser().schoolProvince)
                    startPos=i
            provincePops.add(PopupBean(i,province.value,province.value==MethodManager.getUser().schoolProvince))
        }
        val citys=provinces[startPos].children
        for (i in citys.indices){
            cityPops.add(PopupBean(i,citys[i].value,citys[i].value==MethodManager.getUser().schoolCity))
        }
        ...
        rvList.scrollToPosition(startPos)
