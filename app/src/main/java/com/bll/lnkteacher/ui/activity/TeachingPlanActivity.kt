...
class TeachingPlanActivity : BaseAppCompatActivity() {
    ...
    override fun initView() {
        ...
        setCurrentDateContent(DateUtils.getStartOfDayInMillis())
    ...
    private fun initRecyclerView() {
        ...
        mAdapter?.setOnItemClickListener { adapter, view, position ->
            ...
            if (item.time > 0) {
                ...
                setCurrentDateContent(item.time)
            }
        }
    }
     ...
     * 设置当天内容
     */
    private fun setCurrentDateContent(time: Long) {
        val dateEvent = DateEventDaoManager.getInstance().queryBean(classId, time)
        tv_content_date.text = DateUtils.longToStringDataNoYear(time)
        if (dateEvent != null) {
